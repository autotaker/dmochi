...
do
    ...
    TIME_BOOL="`grep 'Elapsed Time:' $bool.log | sed -e 's/[^:]*: \([0-9.]*\) sec/\1/'`"
    TIME_HORSAT="`grep 'Elapsed Time:' $bool.selective.horsat.log | sed -e 's/[^:]*: \([0-9.]*\)sec/\1/'`"
    TIME_HORSAT2="`grep 'Elapsed Time:' $bool.selective.horsat2.log | sed -e 's/[^:]*: \([0-9.]*\)sec/\1/'`"
    TIME_PREFACE="`grep 'Total:' $bool.selective.preface.log | sed -e 's/[^:]*: \([0-9]*\)ms/\1/'`"
